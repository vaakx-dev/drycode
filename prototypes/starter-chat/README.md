...
Workspace selection, Session creation and resuming, provider and model selection, thinking-level selection, message sending, expandable Tool observation, Run interruption, Settings navigation, and Runtime Reload are useful browser-only stubs. Settings replaces Session navigation with a focused General, Extensions, Providers, and Data sidebar. The collapsed chat sidebar retains labeled Search, Session, Workspace, Settings, and Reload controls, with flyouts for complete Workspace and Session navigation.
...
Open <http://127.0.0.1:4180/>. Interface icons use the pinned Lucide browser CDN. Provider marks use the LobeHub provider icon CDN.
