...
function titlebar() {
  return `<header class="titlebar">
    <span class="app-glyph">${icon("panels-top-left")}</span>
    ...
    <div class="window-controls" aria-label="Window controls">
      <button type="button" aria-label="Minimize">${icon("minus")}</button>
      <button type="button" aria-label="Maximize">${icon("square")}</button>
      <button type="button" aria-label="Close">${icon("x")}</button>
    </div>
  </header>`;
}
...
function workspaceButton() {
  return `<button class="workspace-button" data-action="workspace" title="Select Workspace">
    <span class="workspace-symbol">${icon("folder-open")}</span>
    <span class="button-copy"><b>${escapeHtml(state.workspace.name)}</b><small>${escapeHtml(state.workspace.path)}</small></span>
...
function modelButton() {
  return `<button class="model-button" data-action="model" title="Configure Model">
    <span class="model-symbol">${icon("cpu")}</span>
    <span class="button-copy"><b>${escapeHtml(state.model.name)}</b><small>${escapeHtml(state.model.provider)} - configured</small></span>
    <span class="chevron">${icon("chevron-down")}</span>
  </button>`;
}

function sessionRows() {
  return state.sessions.map((session) => `<button class="session-row ${session.id === state.activeSession ? "selected" : ""}" data-session="${escapeHtml(session.id)}">
    <span class="session-icon">${icon(session.id === state.activeSession ? "message-circle" : "messages-square")}</span>
    <span class="session-row-copy"><b>${escapeHtml(session.title)}</b><small>${escapeHtml(session.summary)}</small></span>
...
function sessionHeader() {
  ...
  return `<div class="session-heading">
    <span class="status-dot ${state.running ? "busy" : "ready"}"></span>
    <div><b>${escapeHtml(session.title)}</b><small>${state.running ? escapeHtml(state.stage) : "Durable Session - ready for input"}</small></div>
...
function toolCards() {
  ...
  return activeTools().map((tool) => `<details class="tool-card" ${tool.status === "running" ? "open" : ""}>
    <summary><span class="tool-mark ${tool.status}">${icon(tool.status === "running" ? "loader-circle" : "circle-check", tool.status === "running" ? "spin" : "")}</span><b>${escapeHtml(tool.name)}</b><span class="tool-detail">${escapeHtml(tool.detail)}</span><span class="tool-status ${tool.status}">${tool.status === "running" ? "Running" : "Completed"}</span></summary>
...
function messageStream() {
  ...
  return `<div class="message-stream">${messages || `<div class="empty-message"><span class="empty-icon">${icon("message-circle")}</span><b>Start this Session</b><span>Ask Drycode to work in ${escapeHtml(state.workspace.name)}.</span></div>`}
    <section class="inline-tools" aria-label="Tool observation"><div class="section-kicker">${icon("activity")}<span>Tool observation</span><span class="muted">${activeTools().length} events</span></div>${toolCards()}</section>
  </div>`;
}
...
function composer() {
  return `<form class="composer" data-composer>
    ...
    <div class="composer-bar"><button type="button" class="attach-button" data-action="attach" title="Add context" aria-label="Add context">${icon("paperclip")}</button><span class="compose-context">${escapeHtml(state.workspace.name)} - ${escapeHtml(state.model.name)}</span><span class="grow"></span>${state.running ? `<button type="button" class="interrupt-button" data-action="interrupt">${icon("square")}<span>Interrupt Run</span></button>` : `<button type="submit" class="send-button"><span>Send</span>${icon("arrow-up")}</button>`}</div>
  </form>`;
}

function navigationView() {
  return `<div class="app-frame"><div>${titlebar()}</div>
    <div class="nav-layout"><nav class="navigation-view">
      <div class="nav-workspace">${workspaceButton()}</div>
      <button class="new-session" data-action="new-session">${icon("plus")}<span>New Session</span></button>
      <div class="nav-section"><span class="section-kicker">Workspace</span><button class="nav-link active" data-action="sessions">${icon("messages-square")}<span>Sessions</span><em>${state.sessions.length}</em></button><button class="nav-link" data-action="workspace">${icon("folder-open")}<span>Change Workspace</span></button></div>
      <div class="session-nav"><div class="section-kicker"><span>Recent Sessions</span><span class="grow"></span><button class="mini-button" data-action="new-session" title="New Session" aria-label="New Session">${icon("plus")}</button></div>${sessionRows()}</div>
      <div class="nav-footer">${modelButton()}<button class="footer-link" data-action="reload">${icon("rotate-ccw")}<span>Reload Runtime</span></button></div>
...
function modal() {
  ...
  if (state.modal === "reload") return `<div class="modal-shade" data-dismiss="true"><section class="dialog" role="dialog" aria-modal="true" aria-labelledby="reload-title"><div class="dialog-title"><span class="dialog-icon amber">${icon("rotate-ccw")}</span><div><span class="section-kicker">Runtime lifecycle</span><h2 id="reload-title">Reload the Runtime?</h2></div></div><p>Drycode will stop the complete UI and Harness Runtime Generation, then start a fresh generation. Durable Sessions remain available.</p><div class="reload-note"><span class="status-dot busy"></span><span>Active work will be interrupted. Your Session record stays safe.</span></div><div class="dialog-actions"><button class="plain-button" data-action="close-modal">Cancel</button><button class="danger-button" data-action="confirm-reload">${icon("rotate-ccw")} Reload Drycode</button></div></section></div>`;
  return `<div class="modal-shade"><section class="dialog reload-progress" role="dialog" aria-modal="true"><span class="dialog-icon amber">${icon("rotate-ccw")}</span><span class="section-kicker">Runtime lifecycle</span><h2>Reloading Runtime...</h2><p>Stopping the current generation and starting a fresh UI and Harness pair.</p><div class="progress-track"><i></i></div><small>Starting Runtime Generation 19</small></section></div>`;
}

function render() {
  document.querySelector("#app").innerHTML = `${navigationView()}${modal()}${state.toast ? `<div class="toast" role="status">${icon("info")}<span>${escapeHtml(state.toast)}</span></div>` : ""}`;
...
function bind() {
  document.querySelectorAll("[data-action]").forEach((element) => element.addEventListener("click", (event) => {
    ...
    if (action === "confirm-reload") return reloadRuntime();
    render();
  }));
